...
def show_dashboard():
    """
    Displays the Appfolio Dashboards.  This function assumes that the data files
    are located in a 'data' directory relative to the script's location.
    This function should be called *after* a user has successfully logged in.
    """

    #  Page layout should be set *once* at the beginning of the app, typically in the main script (login.py in this case)
    #  st.set_page_config(page_title="Appfolio Dashboards", layout="wide")

    # Attempt to import kaleido and handle the ImportError
    try:
        import kaleido  # Required for Plotly image export
    except ImportError:
        st.warning(
            "Kaleido is required for generating static Plotly images.  "
            "Please install it using: `pip install kaleido`"
        )
        #  Important:  Consider whether you want to proceed with *any* of the
        #  dashboard if kaleido is not available.  If not, you might want to
